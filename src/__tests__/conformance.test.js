...
test("v01 field-level AUTO -> active with deterministic override_id", () => {
  const e = engine();
  const ev = e.recordOverride(V.v01.args);
...
test("v02 field-level GATED -> pending", () => {
  const e = engine();
  const ev = e.recordOverride(V.v02.args);
  assert.equal(ev.status, "pending");
  assert.equal(ev.override_id, V.v02.expect.override_id);
});

test("v03 confirm flips pending -> active", () => {
  const e = engine();
  e.recordOverride(V.v02.args);
  e.confirm(V.v03.args.override_id);
  assert.ok(e.eventsFor(V.v03.args.override_id).some(x => x.status === "active"));
});

test("v04 retract -> no active", () => {
  const e = engine();
  e.recordOverride(V.v02.args);
  e.retract(V.v04.args.override_id);
  assert.ok(e.eventsFor(V.v04.args.override_id).every(x => x.status !== "active"));
});

test("v05 identity.merge existing ids -> pending, full schema + value", () => {
  const e = engine();
  const ev = e.recordIdentityMerge(V.v05.args);
  assert.equal(ev.status, "pending");
...
test("v06 identity.split -> pending + engine-minted id BYTE-IDENTICAL to oracle", () => {
  const e = engine();
  const ev = e.recordIdentitySplit(V.v06.args);
  assert.equal(ev.status, "pending");
  assert.equal(ev.override_id, V.v06.expect.override_id);
  assert.equal(ev.value.minted_entity_id, V.v06.expect.minted_entity_id);
});

test("v07 split with agent-supplied new id THROWS before mint (ordering)", () => {
  const e = engine();
  ...
  assert.throws(
    () => e.recordIdentitySplit({ ...V.v07.args, partition_rule: "", agent_supplied_new_id: "attacker-id" }),
...
test("v09 confidence.pin -> active + human-verified", () => {
  const e = engine();
  const ev = e.recordOverride(V.v09.args);
...
test("v10 supersede: prior superseded, new active", () => {
  const e = engine();
  e.recordOverride({ entity_id: "entity-A", override_type: "claim.correct", field_path: "claim.value",
  ...
  e.recordOverride({ entity_id: "entity-A", override_type: "claim.correct", field_path: "claim.value",
    ...
    provenance: { directed_by: "coach", session_ref: "sess-vec" } });
  const events = e.eventsFor(V.v10.args.override_id);
...
test("v11 opaque-id negative: no supplied-value substring in ids", () => {
  const e = engine();
  const name = V.v11.args.name_token;
  const ev = e.recordOverride({ entity_id: "entity-A", override_type: "identity.rename",
  ...
  const sp = e.recordIdentitySplit({ source_entity_id: "entity-A", partition_rule: "by-" + name,
...
test("v12 agent-supplied override_id rejected", () => {
  const e = engine();
  assert.throws(() => e.recordOverride(V.v12.args), err => err instanceof WriteToolError);
});
